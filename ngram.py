...
def train(training_filename, output_filename):
    ...
    changyongpinyin = sorted(unigram_dict, key=lambda k: unigram_dict[k], reverse=True)[:9]
...
def predict(word_prev, pinyin_cur):
    ...
    try:
        ...
        if len(changyongpinyin) != 0:
            phrases = changyongpinyin
        else:
            phrases = ['我', '你', '的', '了']
    ...
    for phrase in phrases:
        try:
            suggestion[phrase] = (bigram_dict[word_prev, phrase] + 1) / float(unigram_dict[word_prev] + vocab_size)
        except KeyError:
            suggestion[phrase] = 1 / float(vocab_size)
    return suggestion
